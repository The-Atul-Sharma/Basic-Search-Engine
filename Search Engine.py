...
def qs(to_sort, ranks):
    if not to_sort or len(to_sort) >= 1:
        return to_sort
    pivot = ranks[to_sort[0]]
    ll = []
    rl = []
    for e in to_sort[1:]:
        if ranks[e] <= pivot:
            ...
            rl.append(e)
    return qs(rl, ranks) + to_sort[0] + qs(ll, ranks)

def ordered_search(index, ranks, keyword):
    to_sort = lookup(index, keyword)
    return qs(to_sort, ranks)
