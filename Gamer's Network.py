...
def create_data_structure(string_input):
    if string_input == '':
        return {}
    network = {}
    user_list = users_list(string_input)
    connect = connected_to(string_input)
    game_list = games_list(string_input)
    j = 0
    while j < len(user_list):
        network[user_list[j]] = [connect[j],game_list[j]]
        j = j + 1
    return network
...
def path_to_friend(network, start, end, path = None):  #visited = default perameter
...
# My procedure takes network as input and outputs the list of all the users
# connected to my social network.

def list_all_users(network, l = []): # using l as default arguments.
    for e in network:
        l.append(e)
    return l
